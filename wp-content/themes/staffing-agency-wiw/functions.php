...
register_nav_menus(
	array(
        ...
        'admin-menu' => esc_html__( 'Administrator', 'staffing-agency-wptheme-wiw' )
	)
);
...
function change_notice( $html ) {
	if ( stristr( $html, 'There were no results found.' ) ) {
		...
		$html = str_replace( 'There were no results found.', 'There are no shifts scheduled for this month.', $html );
	}
	return $html;
}
...
function wp_maintenance_mode() {
  if (!is_user_logged_in()) {
        wp_die('<h2>Undergoing Maintenance</h2><br />Our website currently undergoing maintenance. Sorry for any inconvience.');
    }
}
		...
		if ($query->query_vars['post_type'] == Tribe__Events__Main::POSTTYPE && ! is_singular( 'tribe_events' ) && ! is_tax( Tribe__Events__Main::TAXONOMY ) ) {
			{
				...
				//Filter Client Calendars By Client ID's (Job Sites)
...
// Step 2: Schedule the cron job if it's not already scheduled
function schedule_plugin_activation() {
    if (!wp_next_scheduled('custom_plugin_activation_event')) {
        wp_schedule_event(time(), 'every_fifteen_minutes', 'custom_plugin_activation_event');
    }
}
add_action('wp', 'schedule_plugin_activation');

// Step 3: Define the function to activate the plugin
function activate_wiw_plugin() {
    $plugin_path = 'wiw-api-functions/index.php'; // Path to your plugin

    // Check if the plugin is inactive and activate it
