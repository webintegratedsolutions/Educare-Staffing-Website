...
function getCalendarShiftIds() {
   ...
   $calendar_shift_ids = array();
   foreach($current_shifts as $key => $value) {
	   $shift_meta_shift_id = get_post_meta( $value->ID, 'Shift ID' );
	   $shift_id = $shift_meta_shift_id[0];
	   if ($shift_id){
			if(!in_array($shift_id, $calendar_shift_ids)){
				$calendar_shift_ids[] = $shift_id;
			}
	   }
   }	
